...
public class SqlManager {
    ...
    private void checkStatTable() {
        ...
        try (Connection connection = createConnection()) {
            ...
            String sql = "CREATE TABLE `hb_stat` (" + " `uuid` varchar(36) NOT NULL,"
                    + " `points` int NOT NULL DEFAULT 0," + " `captures` int NOT NULL DEFAULT 0,"
                    + " `kills` int NOT NULL DEFAULT 0," + " `deaths` int NOT NULL DEFAULT 0,"
                    + " UNIQUE KEY `uuid` (`uuid`)" + ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4";
