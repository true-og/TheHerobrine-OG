...
public class VoteCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (wm.getPlayerVotes().get(player) == map) {
            ...
            player.sendMessage(Message.format(ChatColor.GOLD + "Vote received. " + ChatColor.AQUA
                    + wm.getVotingMaps().get(map).getMapData().getName() + ChatColor.GOLD + " now has " + ChatColor.AQUA
                    + wm.getVotingMaps().get(map).getVotes() + ChatColor.GOLD + " votes."));
        }
