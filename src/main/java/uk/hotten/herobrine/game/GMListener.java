...
public class GMListener implements Listener {
    ...
    public void onJoinViaWorld(PlayerChangedWorldEvent event) {
        ...
        if (!player.getWorld().getName().startsWith(gameLobby.getLobbyId())) return;
    ...
    public void onLeaveViaQuit(PlayerQuitEvent event) {
        ...
        if (!gameLobby.getPlayers().contains(player)) return;
    ...
    public void onLeaveViaWorld(PlayerChangedWorldEvent event) {
        ...
        if (player.getWorld().getName().startsWith(gameLobby.getLobbyId())
                && event.getFrom().getName().startsWith(gameLobby.getLobbyId())) return;

        if (player.getWorld().getName().startsWith(gameLobby.getLobbyId())
                && !event.getFrom().getName().startsWith(gameLobby.getLobbyId())) return;

        if (!event.getFrom().getName().startsWith(gameLobby.getLobbyId())) return;
    ...
    public void onPickup(EntityPickupItemEvent event) {
        ...
        if (gameManager.getGameState() != GameState.LIVE) return;
        ...
        for (Player p : gameLobby.getPlayers()) {
            ...
            PlayerUtil.sendTitle(
                    p,
                    "&a&l" + player.getName() + "&3 has picked up the shard!",
                    "&eHelp them return it!",
                    250,
                    3000,
                    250);
        }
        PlayerUtil.sendTitle(
                gameManager.getHerobrine(),
                "&a&l" + player.getName() + "&3 has picked up the shard!",
                "&eMaybe target them first",
                250,
                3000,
                250);
        ...
        gameManager.setTags(
                player, "&d&lShard: ", NamedTextColor.LIGHT_PURPLE, GameManager.ScoreboardUpdateAction.UPDATE);
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            if (player.getInventory().getItemInMainHand().getType() == Material.COMPASS) {
                ...
                Bukkit.getServer()
                        .getScheduler()
                        .runTaskLater(gameManager.getPlugin(), () -> kitCooldown.remove(player), 20);
            }
        }
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (event.getPlayer() == gameManager.getShardCarrier()) {
            for (Player p : gameLobby.getPlayers()) {
                ...
                else p.setCompassTarget(gameLobby.getWorldManager().alter);
            }
        }
    }
    ...
    public void onDamage(EntityDamageByEntityEvent event) {
        ...
        if (event.getEntity() instanceof Player && event.getDamager() instanceof Wolf) {
            Player player = (Player) event.getEntity();
            if (player == gameManager.getHerobrine()) event.setDamage(6);
        ...
        if (event.getEntity() instanceof Wolf && event.getDamager() instanceof Player) {
            ...
            if (player != gameManager.getHerobrine()) event.setCancelled(true);
            return;
        }
        ...
        if (gameManager.getSurvivors().contains(attacker)) { // If attacker is a survivor
            ...
            // Attacking THB
            double damage = gameManager.getHerobrineHitDamage(
            ...
            Bukkit.getServer()
                    .getScheduler()
                    .runTaskLater(gameManager.getPlugin(), () -> player.setVelocity(new Vector(0, 0, 0)), 1);
        } else if (attacker == gameManager.getHerobrine()) {
            PlayerUtil.playSoundAt(attacker.getLocation(), Sound.ENTITY_GHAST_AMBIENT, 1f, 0f);
            double damage = gameManager.getSurvivorHitDamage(
                    attacker.getInventory().getItemInMainHand().getType());
            if (damage != -1) event.setDamage(damage);
        } else {
            event.setCancelled(true);
            ...
            if (event.getCause() == EntityDamageEvent.DamageCause.BLOCK_EXPLOSION
                ...
                Bukkit.getServer()
                        ...
                        .runTaskLater(
                                ...
                                () -> gameManager.getHbLastHit().remove(player),
                                120);
            }
        }
    }
    ...
    public void onDeath(PlayerDeathEvent event) {
        ...
        Bukkit.getServer()
                .getScheduler()
                .runTaskLater(gameManager.getPlugin(), () -> player.spigot().respawn(), 40);
        ...
        if (player == gameManager.getHerobrine()) {
            ...
            if ((player.getKiller() != null && player.getKiller() == gameManager.getHerobrine())
                ...
                Message.broadcast(
                        gameLobby, Message.format("&b" + player.getName() + "&e was killed by &c&lthe HEROBRINE!"));
                gameLobby
                        .getStatManager()
                        .getPointsTracker()
                        .increment(gameManager.getHerobrine().getUniqueId(), 5);
            }
    ...
    public void onRespawn(PlayerRespawnEvent event) {
        if (!gameLobby.getPlayers().contains(event.getPlayer())) return;
        ...
        for (LivingEntity e : event.getAffectedEntities()) {
            if (e instanceof Player) {
                ...
                if (gameManager.getHerobrine() == player
                        || !gameManager.getSurvivors().contains(player))
                    Bukkit.getServer()
                            .getScheduler()
                            .runTaskLater(
                                    gameManager.getPlugin(),
