...
public class GMListener implements Listener {
    ...
    public GMListener(GameManager gm) {
        this.gm = gm;
    }
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        gm.getScoreboards()
                .put(
                        player,
                        ScoreboardLib.createScoreboard(player)
                                .setHandler(new ScoreboardHandler() {
                                    ...
                                    public List<Entry> getEntries(Player player) {
                                        return new EntryBuilder()
                                                .next(ChatColor.AQUA + "Points: " + ChatColor.RESET
                                                        + StatManager.get()
                                                                .getPoints()
                                                                .get(player.getUniqueId()))
                                                .next(ChatColor.AQUA + "Captures: " + ChatColor.RESET
                                                        + StatManager.get()
                                                                .getCaptures()
                                                                .get(player.getUniqueId()))
                                                .next(ChatColor.AQUA + "Kills: " + ChatColor.RESET
                                                        + StatManager.get()
                                                                .getKills()
                                                                .get(player.getUniqueId()))
                                                .next(ChatColor.AQUA + "Deaths: " + ChatColor.RESET
                                                        + StatManager.get()
                                                                .getDeaths()
                                                                .get(player.getUniqueId()))
                                                .build();
        ...
        Message.broadcast(
                Message.format("" + ChatColor.AQUA + player.getName() + " " + ChatColor.YELLOW + "has joined!"));
    ...
    public void onLeave(PlayerQuitEvent event) {
        ...
        if (!silentLeave.contains(player.getUniqueId())) {
            Message.broadcast(
                    Message.format("" + ChatColor.AQUA + player.getName() + " " + ChatColor.YELLOW + "has quit."));
        }
        ...
        if (gm.getPassUser() == player) {
            gm.setPassUser(null);
            Message.broadcast(
                    Message.format(ChatColor.GOLD + player.getName() + " has left and will no-longer be Herobrine."),
    ...
    public void onPickup(EntityPickupItemEvent event) {
        ...
        for (Player p : Bukkit.getServer().getOnlinePlayers()) {
            ...
            PlayerUtil.sendTitle(
                    p,
                    "" + ChatColor.GREEN + ChatColor.BOLD + player.getName() + ChatColor.DARK_AQUA
                            + " has picked up the shard!",
                    ChatColor.YELLOW + "Help them return it!",
                    5,
                    60,
                    5);
        }
        PlayerUtil.sendTitle(
                ...
                "" + ChatColor.GREEN + ChatColor.BOLD + player.getName() + ChatColor.DARK_AQUA
                        + " has picked up the shard!",
                ChatColor.YELLOW + "Maybe target them first",
                5,
                60,
                5);
        ...
        gm.setTags(
                player,
                "" + ChatColor.LIGHT_PURPLE + ChatColor.BOLD + "Shard: ",
        ...
        player.sendMessage(
                Message.format(ChatColor.GOLD + "You have a shard! Take it to the alter (Enchanting Table)!"));
    }
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (gm.getGameState() == GameState.LIVE) {
            if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
                ...
                if (m == Material.ENCHANTING_TABLE) event.setCancelled(true);
                ...
                } else if (m == Material.ITEM_FRAME) event.setCancelled(true);
            }
        }

        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            if (player.getInventory().getItemInMainHand().getType() == Material.COMPASS) {
                if (kitCooldown.contains(player)) return;

                if (gm.getGameState() == GameState.WAITING || gm.getGameState() == GameState.STARTING) {
                    new KitGui(gm.getPlugin(), player).open(false);
                } else if (gm.getGameState() == GameState.LIVE
                        && gm.getSpectators().contains(player)) {
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        if (event.getPlayer() == gm.getShardCarrier()) {
            for (Player p : Bukkit.getServer().getOnlinePlayers()) {
                if (p != event.getPlayer()) p.setCompassTarget(event.getPlayer().getLocation());
                else p.setCompassTarget(WorldManager.getInstance().alter);
            }
        }
    }
    ...
    public void onDamage(EntityDamageByEntityEvent event) {
        ...
        // Allows arrow damage
        if (event.getEntity() instanceof Player
                && event.getDamager() instanceof Arrow) { // If the damaged is a player and damager is an arrow
            Player player = (Player) event.getEntity();
            Player attacker = (Player) ((Arrow) event.getDamager()).getShooter();

            if (!(gm.getSurvivors().contains(attacker)
                    && player == gm.getHerobrine())) { // Evals to true if either a) the attacker isnt a survivor b)
        ...
        if (event.getEntity() instanceof Player && event.getDamager() instanceof Wolf) {
            Player player = (Player) event.getEntity();
            if (player == gm.getHerobrine()) event.setDamage(6);
        ...
        if (event.getEntity() instanceof Wolf && event.getDamager() instanceof Player) {
            ...
            if (player != gm.getHerobrine()) event.setCancelled(true);
            return;
        }
        ...
        if (gm.getSurvivors().contains(attacker)) { // If attacker is a survivor
            ...
            // Attacking THB
            double damage = gm.getHerobrineHitDamage(
    ...
    public void onDamage(EntityDamageEvent event) {
        ...
        if (gm.getSurvivors().contains(player)) {
            if (event.getCause() == EntityDamageEvent.DamageCause.BLOCK_EXPLOSION
                ...
                Bukkit.getServer()
                        .getScheduler()
                        .runTaskLater(gm.getPlugin(), () -> gm.getHbLastHit().remove(player), 120);
            }
        }
    }
    ...
    public void onDeath(PlayerDeathEvent event) {
        ...
        Bukkit.getServer()
                .getScheduler()
                .runTaskLater(gm.getPlugin(), () -> player.spigot().respawn(), 40);

        if (player == gm.getHerobrine()) {
            if (player.getKiller() != null) {
                Message.broadcast(
                        Message.format(ChatColor.AQUA + player.getKiller().getName() + ChatColor.GREEN
                                + " has defeated " + ChatColor.RED + ChatColor.BOLD + "the HEROBRINE!"));
                StatManager.get()
                        .getPointsTracker()
                        .increment(player.getKiller().getUniqueId(), 30);
            }
            ...
            if ((player.getKiller() != null && player.getKiller() == gm.getHerobrine())
                    || gm.getHbLastHit().contains(player)) {
                Message.broadcast(Message.format(ChatColor.AQUA + player.getName() + ChatColor.YELLOW
                        + " was killed by " + ChatColor.RED + ChatColor.BOLD + "the HEROBRINE!"));
                                        ...
                                        if (gm.getHerobrine() == player) {
                                            if (gm.getShardCount() != 3)
                                                PlayerUtil.addEffect(
                                                        ...
                                                        false);
                                            PlayerUtil.addEffect(
                                                    player, PotionEffectType.JUMP, Integer.MAX_VALUE, 1, false, false);
                                            PlayerUtil.addEffect(
                                                    player, PotionEffectType.SPEED, Integer.MAX_VALUE, 1, false, false);
                                        }
                                    },
                                    1);
            }
    ...
    public void onChat(AsyncPlayerChatEvent event) {
        ...
        String endMessage = ChatColor.BLUE + player.getDisplayName() + ChatColor.DARK_GRAY + " » " + ChatColor.RESET
                + event.getMessage();

        if (gm.getGameState() == GameState.WAITING || gm.getGameState() == GameState.STARTING) {
            Message.broadcast("" + ChatColor.YELLOW + points + ChatColor.DARK_GRAY + " ▏ " + rank.getDisplay() + " "
                    + endMessage);
            ...
            if (player == gm.getHerobrine() || gm.getSurvivors().contains(player)) {
                ...
                Message.broadcast("" + ChatColor.YELLOW + points + ChatColor.DARK_GRAY + " ▍ " + ChatColor.DARK_RED
                        + "DEAD " + ChatColor.DARK_GRAY + "▏ " + endMessage);
            }
        }
    }
}
