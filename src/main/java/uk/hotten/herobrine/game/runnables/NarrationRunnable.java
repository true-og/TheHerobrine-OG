...
public class NarrationRunnable extends BukkitRunnable {
    ...
    public void run() {
        ...
        switch (gm.getShardState()) {
            case WAITING: {
                if (firstMsg()) all("&bShard " + (gm.getShardCount() + 1) + " shall be summoned soon!");
                else
                    separate(
                            "&cBEWARE: The Herobrine is still a cloud of smoke!",
                            "&bTry eliminate all the Survivors",
            ...
            case CARRYING: {
                if (firstMsg()) all("&a&l" + gm.getShardCarrier().getName() + "&3 has the shard!");
                else
                    separate(
                            "&aProtect your shard carrier",
                            "&bKill the shard carrier first!",
                            "&eYou're out of the game! Left-click to spectate.");
                break;
            }
            case INACTIVE: {
